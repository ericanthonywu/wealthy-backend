...
type (
	...
	IInvestmentRepository interface {
		...
		TrxInfoSell(IDPersonal uuid.UUID) (data []entities.InvestmentTransaction, err error)
...
func (r *InvestmentRepository) TrxInfo(IDPersonal uuid.UUID) (data []entities.InvestmentTransaction, err error) {
	if err := r.db.Raw(`SELECT tt.amount as price,
       ...
       tt.date_time_transaction as date_transaction
...
func (r *InvestmentRepository) TrxInfoSell(IDPersonal uuid.UUID) (data []entities.InvestmentTransaction, err error) {
	if err := r.db.Raw(`SELECT tt.amount as price,
       ttd.lot,
       ttd.stock_code,
       tmb.broker_name,
